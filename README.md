...
Final image pipeline is implemented in `image_pipeline()`. `class Line` is (partially) implemented to keep track of the polynimial coeficients of the last 8 frames. If there was a line detected in previous frame `search_around_poly()` was invoked to search for line pixels around previously detected polynomial curve. `detected` variable in `class Line` is used to keep track if we detected a line in the previous frame. 
